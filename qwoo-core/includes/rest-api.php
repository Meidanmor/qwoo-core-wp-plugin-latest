...
function update_subscription_cart_token($request)
{
    ...
    $device_id = isset($data['device_id'])
        ? sanitize_text_field($data['device_id'])
        : null;

    $new_cart_token = isset($data['cart_token'])
        ? sanitize_text_field($data['cart_token'])
        : null;

    if (empty($device_id) || empty($new_cart_token)) {
        return new WP_REST_Response([
            'success' => false,
            'message' => 'Missing device_id or cart_token'
        ], 400);
    }

    /*
     * Keep your WooCommerce hardening logic
     */
    ...
    if ($updated === 0) {
        return new WP_REST_Response([
            ...
            'device_id' => $device_id
        ], 200); // not an error, just a no-op
    }

    return rest_ensure_response([
        'success' => true,
        'action' => 'cart_token_updated',
        'device_id' => $device_id
    ]);
}
